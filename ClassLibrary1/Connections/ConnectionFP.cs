...
//            byte[] prBytes = new ByteHelper().Combine(new byte[] { (byte)WorkByte.DLE, (byte)WorkByte.STX,(byte)ConsecutiveNumber}, BytesForSend);
//            prBytes = new ByteHelper().Combine(prBytes, new byte[] {0x00, (byte)WorkByte.DLE, (byte)WorkByte.ETX });
...
//                }
//                if (twait > 10) break;
//                result = new ByteHelper().Combine(result, result_fromPort);
...
//                int positionPacketBegin = new ByteHelper().ByteSearch(result, BytesBegin);
...
//                {
//                    tCurrentPos++;
//                    tPostEnd = new ByteHelper().ByteSearch(result, bytesEnd, tCurrentPos);
//                    if (tPostEnd != -1)
//                    {
...
//            unsigned = new ByteHelper().returnWithOutDublicateDLE(unsigned);
...
//            byte[] tempb = new ByteHelper().returnWithOutDublicateDLE(inBytes);
//            var searchBegin = new ByteHelper().PatternAt(tempb, bytesBegin);
//            if (searchBegin == null)
//                return null;

//            var searchEnd = new ByteHelper().PatternAt(tempb, bytesEnd);
//            if (searchEnd == null)
//                return null;
